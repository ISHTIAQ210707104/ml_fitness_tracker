...
for f in files:
    ...
    if "Gyroscope" in f:
        df["set"] = gyr_set
        gyr_set +=1
        gyr_df = pd.concat([gyr_df, df[["epoch (ms)", "time (01:00)", "elapsed (s)", "x-axis (deg/s)", "y-axis (deg/s)", "z-axis (deg/s)", "participant", "label", "category", "set"]]])
        
    if "Accelerometer" in f:
        df["set"] = acc_set
        acc_set +=1
        acc_df = pd.concat([acc_df, df[["epoch (ms)", "time (01:00)", "elapsed (s)", "x-axis (g)", "y-axis (g)", "z-axis (g)", "participant", "label", "category", "set"]]])
