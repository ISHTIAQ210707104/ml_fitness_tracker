...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r"):
    ...
    if filtered_column not in data.columns:
        raise KeyError(f"The column '{filtered_column}' was not found in data.")

    indexes = argrelextrema(data[filtered_column].values, np.greater)
    peaks = data.iloc[indexes]

    # Plot
    fig, ax = plt.subplots()
    plt.plot(data[filtered_column])
    plt.plot(peaks[filtered_column], "o", color="red")
